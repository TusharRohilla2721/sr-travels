...
function RouteCard({ route }) {
  ...
  const findEstimate = () => {
    ...
    const price = route[vehicle]
    setEstimate(price != null ? { price, label: v.label } : 'onrequest')
  }

  const waMsg = estimate && estimate !== 'onrequest'
    ? `Hi SR Travels! I want a trip from ${route.from_city} to ${route.to_city} in a ${estimate.label}. Estimate shows ₹${estimate.price.toLocaleString('en-IN')}. Please confirm pricing.`
  ...
  return (
    <div className="travel-card" style={{
      ...
      <div style={{ padding: '1.2rem 1.5rem 1.5rem' }}>
        ...
        <select value={vehicle} onChange={e => { setVehicle(e.target.value); setEstimate(null) }}
          style={{
        ...
        {estimate && (
          <div style={{
            ...
            {estimate === 'onrequest' ? (
              ...
              <>
                <span style={{ display: 'inline-block', padding: '0.15rem 0.6rem', background: 'var(--accent)', color: '#fff', fontSize: '0.62rem', letterSpacing: '0.1em', textTransform: 'uppercase', borderRadius: 20, marginBottom: '0.6rem' }}>
                  {estimate.label}
                </span>
                <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.8rem', fontWeight: 400, color: 'var(--accent)', lineHeight: 1, marginBottom: '0.3rem' }}>
                  ≈ ₹{estimate.price.toLocaleString('en-IN')}
                </div>
                <p style={{ fontSize: '0.72rem', color: 'var(--text-muted)', lineHeight: 1.5 }}>
                  Estimate only — final price may vary. Prices are negotiable!
                </p>
              </>
            )}
            <a href={waLink(waMsg)} target="_blank" rel="noopener"
              style={{
                marginTop: '0.8rem', display: 'inline-flex', alignItems: 'center', gap: '0.5rem',
...
export default function Destinations() {
  ...
  useEffect(() => {
    (async () => {
      if (!supabase) {
        setAll(SAMPLE_ROUTES); setFiltered(SAMPLE_ROUTES)
        setIsDemo(true); setLoading(false); return
      }
      const { data, error } = await supabase
        .from('travel_routes').select('*').order('from_city')
  ...
  useEffect(() => {
    ...
    setFiltered(all.filter(r =>
      (!f || r.from_city.toLowerCase().includes(f)) &&
  ...
  return (
    <>
      <nav style={{
        ...
        padding: '1.1rem 3.5rem', background: 'var(--nav-bg)',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <button className="btn-ghost" style={{ padding: '0.5rem 1.2rem', fontSize: '0.72rem' }}
      ...
      <div className="dest-banner">
        ...
        <a href={waLink('Hi! I want to get a quote for a trip.')} target="_blank" rel="noopener"
          style={{
      ...
      <div style={{
        ...
        <h1 style={{
          ...
          color: '#fff', lineHeight: 1.1, marginBottom: '0.8rem'
        }}>Find Your <em style={{ fontStyle: 'italic', color: 'var(--accent-lt)' }}>Perfect</em> Journey</h1>
        <p style={{ fontSize: '0.9rem', color: 'rgba(255,255,255,0.5)', maxWidth: 480, lineHeight: 1.75 }}>
          Browse routes, pick your vehicle, and get an instant estimate. Transparent pricing — no surprises.
        </p>
      </div>
      ...
      <div style={{
        ...
        <input value={fromQ} onChange={e => setFromQ(e.target.value)}
          placeholder="Travelling from… (e.g. Delhi)" style={iStyle} />
        <input value={toQ} onChange={e => setToQ(e.target.value)}
          placeholder="Travelling to… (e.g. Jaipur)" style={iStyle} />
        <button className="btn-ghost" style={{ padding: '0.65rem 1.2rem', fontSize: '0.78rem', whiteSpace: 'nowrap' }}

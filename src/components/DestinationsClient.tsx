...
function RouteCard({ route }: { route: Route }) {
    ...
    const inputStyle: React.CSSProperties = { width: '100%', padding: '0.65rem 1rem', background: 'var(--bg-alt)', border: '1px solid var(--border)', borderRadius: 3, color: 'var(--text)', fontFamily: 'DM Sans, sans-serif', fontSize: '0.88rem', marginBottom: '1rem', appearance: 'none' as const }

    return (
        <div className="travel-card" style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 6, overflow: 'hidden', transition: 'transform 0.3s, box-shadow 0.3s' }}
            onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.transform = 'translateY(-4px)'; (e.currentTarget as HTMLDivElement).style.boxShadow = '0 16px 40px rgba(0,0,0,0.08)' }}
            ...
            <div style={{ padding: '1.2rem 1.5rem 1.5rem' }}>
                ...
                <select value={vehicle} onChange={e => { setVehicle(e.target.value); setEstimate(null) }} style={inputStyle}>
                    ...
                    {VEHICLES.map(v => <option key={v.key} value={v.key} disabled={(route as any)[v.key] == null}>{v.label}{(route as any)[v.key] == null ? ' (on request)' : ''}</option>)}
                </select>
                <button onClick={findEstimate} style={{ width: '100%', padding: '0.85rem', border: '1.5px solid var(--accent)', borderRadius: 3, background: 'transparent', color: 'var(--accent)', fontSize: '0.78rem', letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 500, cursor: 'pointer', transition: 'all 0.3s' }}
                ...
                {estimate && (
                    <div style={{ marginTop: '1rem', padding: '1rem 1.2rem', background: 'var(--tag-bg)', borderRadius: 4, border: '1px solid var(--border)', animation: 'fadeIn 0.3s ease' }}>
                        ...
                        <a href={waLink(waMsg)} target="_blank" rel="noopener noreferrer" style={{ marginTop: '0.8rem', display: 'inline-flex', alignItems: 'center', gap: '0.5rem', background: '#25D366', color: '#fff', padding: '0.55rem 1.2rem', borderRadius: 25, fontSize: '0.75rem', fontWeight: 500, textDecoration: 'none' }}>
...
export default function DestinationsClient() {
    ...
    const iStyle: React.CSSProperties = { flex: 1, minWidth: 180, padding: '0.65rem 1rem', background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 3, color: 'var(--text)', fontFamily: 'DM Sans, sans-serif', fontSize: '0.88rem' }

    return (
        <>
            <div style={{ background: 'var(--bg-alt)', borderBottom: '1px solid var(--border)', padding: '0.8rem 3.5rem', display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
                ...
                <a href={waLink('Hi! I want to get a quote for a trip.')} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', background: '#25D366', color: '#fff', padding: '0.55rem 1.2rem', borderRadius: 25, fontSize: '0.75rem', fontWeight: 500, whiteSpace: 'nowrap', textDecoration: 'none' }}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="white"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.885 3.488" /></svg>
                    Chat on WhatsApp
                </a>
            </div>

            <div style={{ padding: '4rem 3.5rem 3rem', background: 'var(--bg-dark)', position: 'relative', overflow: 'hidden', transition: 'background 0.4s' }}>
                <span className="section-label">Explore Routes</span>
                <h1 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 'clamp(2.4rem, 5vw, 4rem)', fontWeight: 300, color: '#fff', lineHeight: 1.1, marginBottom: '0.8rem' }}>Find Your <em style={{ fontStyle: 'italic', color: 'var(--accent-lt)' }}>Perfect</em> Journey</h1>
            ...
            <div style={{ padding: '1.5rem 3.5rem', background: 'var(--bg-alt)', borderBottom: '1px solid var(--border)', display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap', transition: 'background 0.4s' }}>
                ...
                <button className="btn-ghost" style={{ padding: '0.65rem 1.2rem', fontSize: '0.78rem', whiteSpace: 'nowrap' }} onClick={() => { setFromQ(''); setToQ('') }}>Clear ✕</button>
            </div>

            <section style={{ padding: '3rem 3.5rem 5rem', background: 'var(--bg)', transition: 'background 0.4s' }}>
        ...
        @media (max-width: 768px) {
          section { padding: 3rem 1.5rem 5rem !important; }
          div[style*="3.5rem"] { padding-left: 1.5rem !important; padding-right: 1.5rem !important; }
        }

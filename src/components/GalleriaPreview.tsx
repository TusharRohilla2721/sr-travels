...
const TAGGED_IMAGES: Record<string, string[]> = {
  all: [
...
export default function GalleriaPreview() {
  ...
  useEffect(() => {
    ...
    if (!el) return
    const onDown = (e: MouseEvent | TouchEvent) => { isDragging.current = true; startX.current = 'touches' in e ? e.touches[0].clientX : e.clientX }
    const onUp = (e: MouseEvent | TouchEvent) => {
      if (!isDragging.current) return
      isDragging.current = false
      const endX = 'changedTouches' in e ? e.changedTouches[0].clientX : e.clientX
      const diff = startX.current - endX
      if (Math.abs(diff) > 40) { if (diff > 0) setActive(a => Math.min(a + 1, imgs.length + 1)); else setActive(a => Math.max(a - 1, 0)) }
    }
    el.addEventListener('mousedown', onDown as EventListener); el.addEventListener('mouseup', onUp as EventListener)
    el.addEventListener('touchstart', onDown as EventListener, { passive: true }); el.addEventListener('touchend', onUp as EventListener)
    return () => { el.removeEventListener('mousedown', onDown as EventListener); el.removeEventListener('mouseup', onUp as EventListener); el.removeEventListener('touchstart', onDown as EventListener); el.removeEventListener('touchend', onUp as EventListener) }
  }, [imgs.length])
  ...
  return (
    <>
      <style>{`
        ...
        .gp-tab { padding: 0.35rem 1rem; border-radius: 20px; font-size: 0.65rem; letter-spacing: 0.12em; text-transform: uppercase; cursor: pointer; border: 1px solid rgba(255,255,255,0.15); background: rgba(0,0,0,0.3); color: rgba(255,255,255,0.5); transition: all 0.3s; font-family: 'DM Sans', sans-serif; }
        ...
        .gp-card { position: absolute; width: clamp(220px, 26vw, 340px); height: clamp(300px, 60vh, 520px); border-radius: 16px; overflow: hidden; cursor: pointer; transition: transform 0.55s cubic-bezier(0.22,1,0.36,1), opacity 0.55s, box-shadow 0.55s; box-shadow: 0 8px 32px rgba(0,0,0,0.5); }
        .gp-card img { width: 100%; height: 100%; object-fit: cover; display: block; pointer-events: none; }
        .gp-card-cta { width: clamp(220px, 26vw, 340px); height: clamp(300px, 60vh, 520px); background: var(--accent); border-radius: 16px; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; padding: 2rem; cursor: pointer; }
      ...
      <section className="gp-section" id="galleria-preview">
        <div className="gp-header">
          ...
          <div className="gp-tabs">
            {TABS.map(t => <button key={t} className={`gp-tab${activeTab === t ? ' active' : ''}`} onClick={() => setActiveTab(t)}>{t}</button>)}
          </div>
        </div>
        ...
        <div className="gp-stage" ref={containerRef}>
          ...
          {imgs.map((src, i) => {
            ...
            const tx = offset * 56, tz = -absOff * 120, ry = -offset * 12
            const scale = 1 - absOff * 0.12, opacity = 1 - absOff * 0.22, zIndex = 10 - absOff
            return (
              <div key={src + i} className="gp-card" onClick={() => offset !== 0 && setActive(i + 1)} style={{ transform: `translateX(${tx}%) translateZ(${tz}px) rotateY(${ry}deg) scale(${scale})`, opacity, zIndex, outline: offset === 0 ? '2px solid var(--accent)' : 'none' }}>
          ...
          {(() => {
            ...
            if (absOff > 3) return null
            const tx = offset * 56, tz = -absOff * 120, ry = -offset * 12
            const scale = 1 - absOff * 0.12, opacity = 1 - absOff * 0.22
            return (
              <div className="gp-card-cta" onClick={() => offset === 0 ? router.push('/galleria') : setActive(imgs.length + 1)} style={{ position: 'absolute', transform: `translateX(${tx}%) translateZ(${tz}px) rotateY(${ry}deg) scale(${scale})`, opacity, zIndex: 10 - absOff, transition: 'transform 0.55s cubic-bezier(0.22,1,0.36,1), opacity 0.55s', outline: offset === 0 ? '2px solid rgba(255,255,255,0.4)' : 'none' }}>
                <div style={{ fontSize: '2.5rem', marginBottom: '1rem' }}>✦</div>
                <h3 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.6rem', color: '#fff', marginBottom: '0.8rem' }}>See Full Gallery</h3>
                <p style={{ fontSize: '0.85rem', color: 'rgba(255,255,255,0.8)', lineHeight: 1.6, marginBottom: '1.5rem' }}>Explore all our memories — trips, fleet, team and destinations.</p>
                <button style={{ padding: '0.6rem 1.5rem', background: 'rgba(255,255,255,0.2)', border: '1px solid rgba(255,255,255,0.4)', color: '#fff', borderRadius: 24, fontSize: '0.75rem', letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'pointer' }}>View All</button>
              </div>
            )
          })()}
        ...
        <div className="gp-nav">
          {[...imgs, 'cta'].map((_, i) => <div key={i} className={`gp-dot${active === i + 1 ? ' active' : ''}`} onClick={() => setActive(i + 1)} />)}
        </div>
      </section>
    </>
  )

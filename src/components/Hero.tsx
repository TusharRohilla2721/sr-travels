    ...
    <section id="hero" ref={containerRef} style={{ minHeight: '100vh', display: 'grid', gridTemplateColumns: '1fr 1fr', position: 'relative', overflow: 'hidden', background: 'var(--bg)' }}>
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '8rem 4rem 4rem', position: 'relative', zIndex: 2 }}>
        ...
        <div ref={hintRef} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginTop: '2.5rem', opacity: 0 }}>
          <div style={{ width: 40, height: 1, background: 'var(--accent)', animation: 'scrollLine 2s infinite' }} />
      ...
      <div style={{ position: 'relative', overflow: 'hidden' }}>
        ...
        <div ref={badgeRef} style={{ position: 'absolute', bottom: '3rem', left: '50%', transform: 'translateX(-50%) translateY(20px)', zIndex: 3, background: 'var(--card-bg)', backdropFilter: 'blur(8px)', border: '1px solid var(--border)', padding: '1rem 1.8rem', textAlign: 'center', borderRadius: 4, minWidth: 180, opacity: 0 }}>
      ...
      <style>{`
        ...
        @media (max-width: 768px) {
          ...
          #hero > div:last-child { margin: 0 1.25rem 3rem !important; height: 50vh !important; min-height: 380px !important; border-radius: 16px !important; position: relative !important; overflow: hidden !important; }
        }
      `}</style>
    </section>
  )

...
export default function AboutPreview() {
    ...
    return (
        ...
        .about-preview-card { flex: 0 0 280px; height: 300px; border-radius: 12px; padding: 1.8rem 1.5rem; background: var(--card-bg); border: 1px solid var(--border); scroll-snap-align: start; transition: transform 0.3s, box-shadow 0.3s; cursor: grab; display: flex; flex-direction: column; justify-content: center; }
        .about-preview-card:hover { transform: translateY(-5px); box-shadow: 0 10px 30px rgba(0,0,0,0.1); }
        @media (max-width: 768px) {
          .about-preview-card { flex: 0 0 78vw; height: 260px; padding: 1.4rem 1.3rem; }
          #about-preview .about-preview-header { padding: 0 1.5rem !important; }
        }
      `}</style>
            <section id="about-preview" style={{ padding: '6rem 0', background: 'var(--bg-alt)', transition: 'background 0.4s', display: 'flex', flexDirection: 'column' }}>
                <div className="about-preview-header" style={{ marginBottom: '2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '1rem', padding: '0 1.5rem' }}>
                    <div>
                ...
                <div className="about-scroll" ref={scrollRef} style={{ display: 'flex', gap: '1.2rem', overflowX: 'auto', scrollSnapType: 'x mandatory', WebkitOverflowScrolling: 'touch', scrollBehavior: 'smooth', padding: '1rem 1.5rem 3rem 1.5rem' }}>
                    {ABOUT_CARDS.map((item, idx) => (
                        <div key={idx} className="about-preview-card">
                            <span style={{ display: 'inline-block', alignSelf: 'flex-start', padding: '0.2rem 0.6rem', border: '1px solid var(--accent)', color: 'var(--accent)', borderRadius: 20, fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.9rem' }}>{item.tag}</span>
                            <h3 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.5rem', color: 'var(--text)', marginBottom: '0.7rem' }}>{item.title}</h3>
